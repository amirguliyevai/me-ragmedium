...
const { GatewayClient } = require(path.join(DIST, 'client-9y48GCMh.js'));
const GATEWAY_TOKEN = '18887c51a4bc4055bbbe047966f108bb0b2e9d91f3b76d0f9d4b7287cb50cb70';
...
let gwClient = null, gwReady = false, gwSeq = 0;
const gwPending = {};
...
function connectGateway() {
  gwClient = new GatewayClient({
    ...
    clientInfo: { id: 'dash-server', version: '1.0', platform: 'linux', mode: 'cli' },
    scopes: ['operator.read', 'operator.write', 'operator.admin'],
    role: 'operator'
  });
  const orig = gwClient.handleMessage.bind(gwClient);
  gwClient.handleMessage = (data) => {
    try {
      ...
      if (f.type === 'res' && f.id && gwPending[f.id]) {
        gwPending[f.id].resolve(f);
        delete gwPending[f.id];
      }
    } catch(e) {}
    orig(data);
  };
...
function gwRpc(method, params, timeoutMs) {
  return new Promise((resolve, reject) => {
    if (!gwClient) return reject(new Error('Gateway not initialized'));
    const id = String(++gwSeq);
    gwPending[id] = { resolve, reject };
    gwClient.request(method, params, id);
    setTimeout(() => {
      if (gwPending[id]) {
        delete gwPending[id];
...
function defaultState(){ return {
  ...
  brain:{ activeThread:'general', threads:defaultThreads(), reminders:[], calendar:[], timetable:defaultTimetableConfig(), commands:[], sync:{ lastRefresh:null, sources:{} }, notes:[], content:defaultContent() },
  views:{ activeBusiness:'rema-exteriors' }, updatedAt:nowISO()
}; }
...
function threadContext(state,thread){
  ...
  return [`Thread: ${thread.title} — ${thread.scope}`, `Relevant businesses: ${businesses.map(b=>`${b.name}(${b.status}/${b.health})`).join('; ')||'all'}`, `Selected skills: ${skillCtx}`, `Open tasks: ${tasks.map(t=>`${t.project||'Execution'}: ${t.text}`).join(' | ')||'none'}`, `Leads: ${leads.map(l=>`${l.company||l.name||'Lead'} [${l.stage||'new'}] ${l.nextAction||''}`).join(' | ')||'none'}`, `Reminders: ${reminders.map(r=>`${r.text}${r.dueAt?' @ '+r.dueAt:''}`).join(' | ')||'none'}`, `Recent ops: ${state.ops.inbox.slice(0,8).map(x=>x.text).join(' | ')||'none'}`].join('\n').slice(0,14000);
}
...
connectGateway(); // Connect gateway for web_search tool
...
async function sendToAssistant(state,thread,message,model){
  ...
  // Route through the OpenClaw Gateway — this gives the dashboard AI full
  // Kaneki capabilities: SOUL.md/USER.md/MEMORY.md context, web search,
  // file operations, memory recall, all skills/tools.
  const res = await gwClient.request('agent', {
    message: message,
  ...
  const reply = res?.result?.payloads?.[0]?.text
    ...
    || (res?.status === 'ok' ? '(completed)' : 'Error: ' + (res?.error || 'unknown gateway error'));
...
function runAssistantBackground(tid, snapshot, thread, text, model){
  sendToAssistant(snapshot, thread, text, model).then(out=>{
    try {
      ...
      // Find and remove the pending assistant message (don't depend on snapshot id — use pos)
    ...
    try {
      ...
      const pIdx = (t2.messages||[]).findLastIndex(m=>m.role==='assistant'&&m.pending);
      if(pIdx>=0) t2.messages.splice(pIdx,1);
      t2.messages.push({id:id('msg'),role:'system',text:'Assistant error: '+e.message,at:nowISO(),threadId:tid});
      writeState(s2);
...
async function route(req,res){
  ...
  if(u.pathname==='/'||u.pathname==='/index.html'||!u.pathname.startsWith('/api/')) return send(res,200,fs.readFileSync(path.join(ROOT,'index.html'),'utf8'),'text/html');
...
http.createServer((req,res)=>route(req,res).catch(e=>send(res,500,{error:'server_error',detail:String(e.message||e)}))).listen(PORT,HOST,()=>{
